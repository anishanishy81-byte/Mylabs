...
const PatientsGP = () => {
  ...
  const filterAndSortEntries = () => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(entry => 
        ...
        entry.gpNumber?.toLowerCase().includes(term) ||
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm('Are you sure you want to delete this entry?')) {
      try {
        await deleteDoc(doc(db, 'gpEntries', id));
        await loadEntries();
  ...
  const downloadCSV = () => {
    const csvData = filteredEntries.map(entry => ({
      ...
      'Test': entry.test || 'PAP SMEAR',
      'Entered By': entry.enteredBy
    }));

    const ws = XLSX.utils.json_to_sheet(csvData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Patients GP');
    XLSX.writeFile(wb, `patients_gp_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
      ...
      <div className="patients-container">
        ...
        <div className="patients-header">
          <div className="header-content">
            <h1 className="patients-title">Patients GP</h1>
            <p className="patients-subtitle">Gynecological Pathology Patient Records</p>
          </div>
          <div className="header-stats">
            <div className="stat-badge gp">
        ...
        <div className="search-actions-bar">
          <div className="search-box">
            ...
            <input
              ...
              placeholder="Search by name, GP number, hospital, doctor, or date..."
              value={searchTerm}
          ...
          <button className="btn-download gp" onClick={downloadCSV}>
            <svg viewBox="0 0 20 20" fill="currentColor">
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th onClick={() => handleSort('gpNumber')}>
                    <div className="th-content">
                      GP Number
                      <SortIcon column="gpNumber" />
                    </div>
                  </th>
                ...
                {currentEntries.length > 0 ? (
                  currentEntries.map((entry) => (
                    <tr key={entry.id}>
                      <td>{entry.date}</td>
                      <td>
                        <span className="gp-number">{entry.gpNumber}</span>
                      </td>
